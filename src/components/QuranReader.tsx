...
export const QuranReader: React.FC<QuranReaderProps> = ({
  ...
  // Collapsible Surah & Tajwid Drawer State (Default: FALSE / HIDDEN when opening surah)
  const [showDaftarSurahTajweed, setShowDaftarSurahTajweed] = useState<boolean>(false);
  const [collapsibleTab, setCollapsibleTab] = useState<"surah" | "tajweed">("surah");
  ...
  // Load Surah & Verses & ensure drawer is collapsed by default
  useEffect(() => {
    const meta = SURAHS.find((s) => s.number === surahNumber) || SURAHS[0];
    setCurrentSurah(meta);
    setLoading(true);
    setShowDaftarSurahTajweed(false); // Collapse panel when switching surah
  ...
  // Filtered Surahs for Collapsible Drawer
  const filteredDrawerSurahs = SURAHS.filter((s) =>
    s.transliteration.toLowerCase().includes(drawerSearchQuery.toLowerCase()) ||
    s.translationId.toLowerCase().includes(drawerSearchQuery.toLowerCase()) ||
    s.number.toString() === drawerSearchQuery.trim()
  );

  return (
    <div className="space-y-6 max-w-5xl mx-auto px-1 sm:px-0">
      {/* Top Reader Controls Bar */}
      <div className="soft-card p-3 sm:p-4 flex flex-col md:flex-row items-center justify-between gap-3 sticky top-16 sm:top-20 z-30 shadow-md backdrop-blur-md bg-white/95 dark:bg-[#12221A]/95 border border-emerald-100 dark:border-[#1E3A2C]">
        
        {/* Navigation & Surah Dropdown Row */}
        <div className="flex items-center gap-1.5 sm:gap-2 w-full md:w-auto justify-between md:justify-start overflow-x-auto">
          {onBackToSurahList && (
            <button
              onClick={onBackToSurahList}
              className="flex items-center gap-1 px-2.5 py-1.5 rounded-xl bg-[#EAF4EE] dark:bg-[#1E3A2C] text-[#0F5132] dark:text-[#74C69D] hover:bg-[#D8F3DC] font-bold text-xs transition-colors border border-emerald-200/80 dark:border-[#2A4D3A] shrink-0"
          ...
          <button
            ...
            className="p-1.5 sm:p-2 rounded-xl border border-slate-200 dark:border-[#1E3A2C] disabled:opacity-30 hover:bg-slate-100 dark:hover:bg-[#1E3A2C] transition-colors shrink-0"
          ...
          <select
            ...
            className="bg-[#F4FDF8] dark:bg-[#1E3A2C] border border-slate-200 dark:border-[#2A4D3A] rounded-xl px-2.5 py-1.5 font-bold text-xs sm:text-sm text-[#0F5132] dark:text-[#74C69D] focus:outline-none focus:ring-2 focus:ring-[#2D6A4F] max-w-[160px] sm:max-w-xs truncate"
          >
            {SURAHS.map((s) => (
          ...
          {/* Toggle Collapsible Daftar Surah & Tajwid Button */}
          <button
            onClick={() => setShowDaftarSurahTajweed(!showDaftarSurahTajweed)}
            className={`flex items-center gap-1 px-3 py-1.5 rounded-xl text-xs font-bold transition-all border shrink-0 ${
              showDaftarSurahTajweed
                ? "bg-[#1B4332] text-white border-[#1B4332] dark:bg-[#2D6A4F] shadow-xs"
                : "bg-emerald-50 dark:bg-[#1E3A2C] text-[#1B4332] dark:text-[#74C69D] border-emerald-200 dark:border-[#2A4D3A] hover:bg-emerald-100"
            }`}
            title="Sembunyikan / Tampilkan Daftar Surah & Tajwid"
          >
            <span>Daftar & Tajwid</span>
            {showDaftarSurahTajweed ? <ChevronUp className="w-3.5 h-3.5" /> : <ChevronDown className="w-3.5 h-3.5" />}
          </button>
        </div>

        {/* Reader Display Toggles */}
        <div className="flex items-center gap-1.5 flex-wrap justify-center text-[11px] sm:text-xs font-semibold w-full md:w-auto">
          {/* Font Size Adjuster */}
          <div className="flex items-center gap-1 bg-[#FAF8F5] dark:bg-[#1E3A2C] px-2 py-1 rounded-xl border border-slate-200 dark:border-[#2A4D3A]">
            ...
            <span className="w-5 text-center font-bold text-slate-700 dark:text-slate-200">{arabicFontSize}</span>
            <button
          ...
          >
            ...
            <span>Tajwid {showTajweedColors ? "ON" : "OFF"}</span>
          </button>
      ...
      {/* Collapsible Daftar Surah & Tajwid Drawer Panel */}
      <AnimatePresence>
        {showDaftarSurahTajweed && (
          <motion.div
            ...
            className="overflow-hidden soft-card p-4 bg-white dark:bg-[#12221A] border border-emerald-200 dark:border-[#1E3A2C] space-y-4 shadow-xl rounded-2xl"
          >
            {/* Header & Subtab Selector */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-200 dark:border-[#1E3A2C]">
              <div className="flex items-center gap-1.5 bg-[#FAF8F5] dark:bg-[#1E3A2C] p-1 rounded-xl border border-slate-200 dark:border-[#2A4D3A] w-fit">
                <button
                  onClick={() => setCollapsibleTab("surah")}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
                    collapsibleTab === "surah"
                      ? "bg-[#1B4332] text-white shadow-xs dark:bg-[#2D6A4F]"
                      : "text-slate-600 dark:text-slate-300"
                  }`}
                >
                  <BookOpen className="w-3.5 h-3.5" />
                  <span>Cari / Indeks Surah</span>
                </button>

                <button
                  onClick={() => setCollapsibleTab("tajweed")}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
                    collapsibleTab === "tajweed"
                      ? "bg-[#1B4332] text-white shadow-xs dark:bg-[#2D6A4F]"
                      : "text-slate-600 dark:text-slate-300"
                  }`}
                >
              ...
              <button
                onClick={() => setShowDaftarSurahTajweed(false)}
                className="text-xs font-bold text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 flex items-center gap-1 self-end sm:self-auto"
              >
                <span>Tutup Collapse</span>
                <X className="w-4 h-4" />
              </button>
            </div>

            {/* TAB 1: QUICK SURAH INDEX & SEARCH */}
            {collapsibleTab === "surah" && (
              <div className="space-y-3">
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="Ketik nama surah atau nomor (cth: Yasin, Kahf, 18)..."
                    className="w-full pl-9 pr-3 py-2 rounded-xl text-xs bg-[#FAF8F5] dark:bg-[#1E3A2C] border border-slate-200 dark:border-[#2A4D3A] focus:outline-none focus:ring-2 focus:ring-[#2D6A4F] text-slate-800 dark:text-slate-200"
                  />
                </div>

                <div className="max-h-60 overflow-y-auto grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 pr-1">
                  {filteredDrawerSurahs.map((s) => (
                    <button
                      key={s.number}
                      onClick={() => {
                        onSelectSurah(s.number);
                        setShowDaftarSurahTajweed(false);
                      }}
                      className={`p-2 rounded-xl border text-left flex items-center justify-between text-xs transition-all ${
                        s.number === surahNumber
                          ...
                          : "bg-white dark:bg-[#12221A] text-slate-800 dark:text-slate-200 border-slate-200 dark:border-[#1E3A2C] hover:border-[#2D6A4F]"
                      }`}
                    >
                      <div className="truncate">
                        <span className="font-extrabold text-[10px] opacity-75 mr-1">{s.number}.</span>
                        <span>{s.transliteration}</span>
                      </div>
                      <span className="font-arabic text-sm shrink-0 ml-1">{s.name}</span>
                    </button>
                  ))}
                </div>
              </div>
            ...
            {/* TAB 2: TAJWEED QUICK RULES GUIDE */}
            {collapsibleTab === "tajweed" && (
              <div className="max-h-64 overflow-y-auto space-y-3 pr-1">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                  {TAJWEED_RULES.map((rule) => (
                    <div
                      key={rule.id}
                      className="p-3 rounded-xl border border-slate-200 dark:border-[#1E3A2C] bg-[#FAF8F5] dark:bg-[#1E3A2C]/40 space-y-1 text-xs border-l-4"
                      style={{ borderLeftColor: rule.colorTag }}
                    >
                      <div className="flex items-center justify-between">
                        <span className="font-bold text-slate-800 dark:text-slate-100">{rule.title}</span>
                        <span className="font-arabic text-base font-bold text-[#1B4332] dark:text-[#74C69D]">
                          {rule.arabicName}
                        </span>
                      </div>
                      ...
                      <div className="flex items-center justify-between pt-1 text-[10px] text-slate-500">
                        <span className="font-semibold text-emerald-700 dark:text-emerald-400">
                          Contoh: {rule.exampleTransliteration}
                        </span>
                        <span className="font-arabic text-sm">{rule.exampleArabic}</span>
                      </div>
                    </div>
                  ))}
                </div>
